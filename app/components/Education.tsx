...
function EducationCard({ entry }: EducationCardProps) {
  ...
  return (
    <article className='flex flex-col gap-2 rounded-lg border border-[var(--color-surface)] bg-[var(--color-surface)] p-6 transition-colors hover:border-[var(--color-accent)]'>
      {/* Institution */}
      <h3 className='text-lg font-semibold text-[var(--color-text)]'>{institution}</h3>
        ...
        <div className='flex flex-col gap-4 sm:flex-row sm:flex-wrap'>
          {education.map((entry) => (
            <motion.div
              ...
              variants={fadeUpVariants}
              className='min-w-[260px] sm:flex-1'
            >
